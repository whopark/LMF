...
// Item-shape the routes' toResponse() expects. Shared fields come from item_content unless
// the per-area row carries an override (COALESCE). _id is synthesized (PG has no ObjectId);
// editing stays on Mongo until Phase 4b, so the synthetic id is read-only display only.
